...
class Queue extends Array {
    ...
    add(track, offset) {
        if (!Utils_1.TrackUtils.validate(track)) {
            const debugInfo = Array.isArray(track)
                ? `Array with ${track.length} items`
                : typeof track;
            // Only log in debug mode
            if (process.env.DEBUG === 'erela.js') {
                console.error('[Erela.js Queue] Validation failed for:', debugInfo);
            }
            throw new RangeError('Track must be a "Track" or "Track[]". Validation failed.');
        }
        if (!this.current) {
