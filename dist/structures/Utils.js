...
class TrackUtils {
    ...
    static validate(trackOrTracks) {
        ...
        if (Array.isArray(trackOrTracks)) {
            ...
            if (trackOrTracks.length === 0)
                return true;
            // Check each track in the array
            for (let i = 0; i < trackOrTracks.length; i++) {
                const track = trackOrTracks[i];
                if (!(track[TRACK_SYMBOL] || track[UNRESOLVED_TRACK_SYMBOL])) {
                    // Only log detailed errors if DEBUG env is set
                    if (process.env.DEBUG === 'erela.js') {
                        console.error(`[Erela.js] Track validation failed at index ${i}:`, {
                            hasTrackSymbol: !!track[TRACK_SYMBOL],
                            hasUnresolvedSymbol: !!track[UNRESOLVED_TRACK_SYMBOL],
                            trackKeys: track ? Object.keys(track).slice(0, 10) : [],
        ...
        const isValid = (trackOrTracks[TRACK_SYMBOL] ||
            trackOrTracks[UNRESOLVED_TRACK_SYMBOL]) === true;
        if (!isValid && process.env.DEBUG === 'erela.js') {
            console.error('[Erela.js] Single track validation failed:', {
                hasTrackSymbol: !!trackOrTracks[TRACK_SYMBOL],
                hasUnresolvedSymbol: !!trackOrTracks[UNRESOLVED_TRACK_SYMBOL],
                trackKeys: trackOrTracks ? Object.keys(trackOrTracks).slice(0, 10) : [],
                trackType: typeof trackOrTracks
            });
        }
        return isValid;
    }
    ...
    static build(data, requester) {
        ...
        try {
            ...
            const trackInfo = data.info || data;
            const encodedTrack = data.encoded || data.track;
            if (!encodedTrack) {
                throw new Error('Missing encoded track data');
            }
            const track = {
                track: encodedTrack,
                ...
                duration: trackInfo.length || trackInfo.duration,
                isSeekable: trackInfo.isSeekable !== undefined ? trackInfo.isSeekable : true,
                isStream: trackInfo.isStream !== undefined ? trackInfo.isStream : false,
                uri: trackInfo.uri,
                thumbnail: trackInfo.uri?.includes("youtube")
                    ? `https://img.youtube.com/vi/${trackInfo.identifier}/default.jpg`
                    : null,
                displayThumbnail(size = "default") {
                    const finalSize = SIZES.find((s) => s === size) ?? "default";
                    return this.uri?.includes("youtube")
                        ? `https://img.youtube.com/vi/${trackInfo.identifier}/${finalSize}.jpg`
                        : null;
                },
                requester,
            };
            ...
            console.error('[Erela.js] Error building track:', error);
