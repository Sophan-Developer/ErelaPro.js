...
export class Manager extends EventEmitter {
  ...
  public search(
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const res = await node
        .makeRequest<LavalinkResult>(endpoint)
        .catch(err => {
      ...
      let normalizedLoadType: LoadType = res.loadType as LoadType;
      if (node.options.version === 4) {
        if (res.loadType === 'track') normalizedLoadType = 'TRACK_LOADED';
        else if (res.loadType === 'playlist') normalizedLoadType = 'PLAYLIST_LOADED';
        else if (res.loadType === 'search') normalizedLoadType = 'SEARCH_RESULT';
        else if (res.loadType === 'empty') normalizedLoadType = 'NO_MATCHES';
        else if (res.loadType === 'error') normalizedLoadType = 'LOAD_FAILED';
      }

      const result: SearchResult = {
        loadType: normalizedLoadType,
        exception: res.exception ?? null,
        tracks: tracks.map((track: TrackData) =>
  ...
  public decodeTracks(tracks: string[]): Promise<TrackData[]> {
    return new Promise(async (resolve, reject) => {
      ...
      const res = await node.makeRequest<TrackData[]>(endpoint, r => {
        r.method = "POST";
...
export interface LavalinkResult {
  // V3 format
  tracks?: TrackData[];
  loadType: LoadType | 'track' | 'playlist' | 'search' | 'empty' | 'error';
  exception?: {
    /** The message for the exception. */
    message: string;
    /** The severity of exception. */
