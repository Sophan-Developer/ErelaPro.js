...
export class Node {
  ...
  /** The session ID for Lavalink V4 */
  ...
  public connect(): void {
    ...
    const wsPath = this.options.version === 3 ? "" : "/v4/websocket";
    const wsUrl = `ws${this.options.secure ? "s" : ""}://${this.address}${wsPath}`;
  ...
  public async makeRequest<T>(endpoint: string, modify?: ModifyRequest): Promise<T> {
    ...
    const response = await request.body.json() as T;
    return response;
  }
  ...
  protected error(error: Error): void {
    if (!error) return;
    console.error(`[Erela.js] Node ${this.options.identifier} error:`, error.message);
    this.manager.emit("nodeError", this, error);
  }
  ...
  protected message(d: Buffer | string): void {
    ...
    switch (payload.op) {
      case "stats":
        ...
        break;
      case "playerUpdate":
        const guildId = this.options.version === 4 ? payload.guildId : payload.guildId;
      ...
      case "event":
        ...
        break;
      case "ready":
        // V4 ready event - capture session ID
  ...
  protected trackEnd(player: Player, track: Track, payload: TrackEndEvent): void {
    // Normalize V4 lowercase reasons to V3 uppercase format
    let reason: TrackEndReason = payload.reason;
    if (this.options.version === 4) {
      const reasonMap: { [key: string]: TrackEndReason } = {
        'finished': 'FINISHED',
      ...
      reason = reasonMap[reason] || (reason.toUpperCase() as TrackEndReason);
    }
    
    // If a track had an error while starting
    if (["LOAD_FAILED", "CLEANUP"].includes(reason)) {
...
export interface NodeOptions {
  ...
  /** The Lavalink version (3 or 4). Defaults to 4. */
  version?: 3 | 4;
}
